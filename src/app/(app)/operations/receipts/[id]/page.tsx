...
export default function ReceiptDetailPage() {
  ...
  useEffect(() => {
    getDocument("receipts", id).then((r) => setReceipt(r as unknown as Receipt));
  }, [id]);
  ...
  async function handleValidate() {
    ...
    try {
      for (const item of receipt.items) {
        const prod = await getDocument("products", item.productId) as Product | null;
        if (prod) {
      ...
      await updateDocument("receipts", id, { status: STATUS.DONE });
  ...
  if (!receipt) return <div className="flex-1"><Topbar title="Receipt" /><div className="p-6"><div className="h-40 rounded-xl bg-slate-100 animate-pulse" /></div></div>;

  return (
    <div className="flex-1">
      <Topbar title="Receipt Detail" />
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-semibold text-slate-900 mt-0.5">{receipt.supplier}</p>
            </div>
            <Badge variant="outline" className={STATUS_COLORS[receipt.status]}>{receipt.status}</Badge>
          </div>
          ...
          <div className="flex gap-3">
            {receipt.status !== STATUS.DONE && receipt.status !== STATUS.CANCELED && (
              <Button onClick={handleValidate} className="bg-green-600 hover:bg-green-700" disabled={loading}>
                {loading ? "Validating..." : "✓ Validate & Update Stock"}
              </Button>
            )}
